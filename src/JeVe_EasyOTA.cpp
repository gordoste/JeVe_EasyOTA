...
void EasyOTA::onEnd(ArduinoOTAClass::THandlerFunction fn) {
    on_end = fn;
}
void EasyOTA::onError(ArduinoOTAClass::THandlerFunction_Error fn) {
    on_error = fn;
}
...
int EasyOTA::setupOTA(unsigned long now) {
    ...
    ArduinoOTA.onStart([this]() { showMessage("OTA starting...", 1); onStart(); });
    ArduinoOTA.onEnd([this]() { showMessage("OTA done.Reboot...", 1); onEnd(); });
...
void EasyOTA::showMessage(const String &message, int line_nr) {
    if (on_message) on_message(message, line_nr);
}
...
void EasyOTA::onError(ota_error_t err) {
